    ...
    try:
        results = pd.read_sql_query("""
            SELECT e.Experiment_name, s.Model, s.Accuracy,
                   ...
                   s.cross_val_score, s.Params,s.confusion_matrix
            FROM Experiments e
    ...
    try:
        results = pd.read_sql_query("""
            SELECT e.Experiment_name, s.Model, s.MSE,
                   s.MAE, s.R2, s.RMSE, s.cv_score, s.Params
            FROM Experiments e
...
if page == "Model Comparison":
    ...
    if select_problem == "Classification":
        if df.empty:
            ...
            fig = px.scatter(
                ...
                size="F1_Score", color="Model", text="Model", template=chart_theme
            )
            st.plotly_chart(fig, use_container_width=True)
            ...
            option=st.selectbox("Select Model to view confusion matrix",filtered_df["Model"])
            cmf=filtered_df[filtered_df["Model"]==option]
            try:
            ...
            if selected_model_params:
                row = filtered_df[filtered_df["Model"] == selected_model_params].iloc[0]
    ...
    # ---- REGRESSION ----
        ...
        if df_reg.empty:
            ...
            fig = px.bar(melted_reg, x="Model", y="Value", color="Metric", barmode="group", template=chart_theme)
            st.plotly_chart(fig, use_container_width=True)

            st.subheader("📊 R2 Score Comparison")
            fig = px.bar(filtered_reg, x="Model", y="R2", color="R2", template=chart_theme)
            st.plotly_chart(fig, use_container_width=True)

            st.subheader("📉 MSE vs MAE")
            fig = px.scatter(
                filtered_reg, x="MSE", y="MAE",
                size="RMSE", color="Model", text="Model", template=chart_theme
            )
            st.plotly_chart(fig, use_container_width=True)

            st.subheader("📊 Cross Validation Score")
            fig = px.bar(filtered_reg, x="Model", y="cv_score", color="cv_score", template=chart_theme)
            st.plotly_chart(fig, use_container_width=True)

            st.markdown("---")
            st.subheader("📋 Copy Model Parameters")
            reg_models = filtered_reg["Model"].unique()
            selected_reg_model = st.selectbox("Select Model to Copy Params", reg_models)
            if selected_reg_model:
                row = filtered_reg[filtered_reg["Model"] == selected_reg_model].iloc[0]
                model_params = row["Params"] if "Params" in row.index else {}
                st.code(json.dumps(model_params, indent=4), language="json")
